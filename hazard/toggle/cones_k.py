...
forwardboundstr = input("How many steps forward would you like to group into cones? ")
while not(forwardboundstr.isdigit()):
  forwardboundstr = input("ENTER A DIGIT!\nHow many steps forward would you like to group into cones? ")
forwardbound = int(forwardboundstr)

foldername = str(totest) + "_" + ts + "_" + str(upperbound) + "_" + str(forwardbound)
...
for logline in log: 
  ...
  while not(finished):
    ...
    with open(abrfile) as abr:
      ...
      abrindex = 0
      for abrline in abr:
        if abrindex == forwardbound:
          break
        else:
          abrsplit = abrline.split(" ")
          coneprefix = coneprefix + abrsplit[1] + "." + abrsplit[3].rstrip("\n") + ">"
          # coneprefix = coneprefix + "_" + abrsplit[1] + "-" + abrsplit[3].rstrip("\n")
          abrindex = abrindex + 1
    with open(abrfile) as abr:
      foundcone = False
      conefilename = conefolder + "/"
      for a in range(0,conefilecount):
        with open(conefolder + "/cone" + str(conefilecount) + ".txt", 'r') as conef:
          if coneprefix in conef.readline():
            foundcone = True
            conefilename = conefolder + "/cone" + str(conefilecount) + ".txt"
      if not(foundcone):
        conefilecount = conefilecount + 1
        conefilename = conefolder + "/cone" + str(conefilecount) + ".txt"
        with open(conefilename, 'a') as cone:
          cone.write(coneprefix + "\n:::::::::::::::::::::::::::::::::::::::::::::::\n")
      with open(conefilename, 'a') as cone:
        for abrline in abr:
          abrsplit = abrline.split(" ")
          cone.write("t=" + abrsplit[1] + "_l=" + abrsplit[3].rstrip("\n") + " -> ")
        cone.write("X\n")
